...
func New(pushConfig *PushConfig, remote *config.StorageConfig) (*Remotestorage, error) {
	...
	if remote.GDrive.ApiKeyJson != "" {
		...
		if err != nil {
			return remotestorage, err
		}
		remotestorage.gDrive = gDrive
	}
...
func (r *Remotestorage) PushBackup() *PushReport {
	...
	for _, TO := range r.PushTO {
		...
		case "gDrive":
			...
			if err := r.gDrive.UploadFile(r.Upload, r.RemoteDir); err != nil {
